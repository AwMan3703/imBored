...
class SkyShader extends FlxShader {
    @:glFragmentSource('
        #pragma header
        uniform float uTime;

        uniform vec2 uResolution;
        /*uniform*/ float iHour = uTime/2.*(.15);
    ...
    public function new() {
        super();
        this.uResolution.value = [860.0, 640.0];// somehow this doesn't work -> [FlxG.width, FlxG.height];
        this.uTime.value = [0.0];
        //this.iHour.value = ; // insert DateTime() here if MABYE intellisense
                               // decides to start working PLEASE
    }
}